...
from thirdparty.monogs.utils.slam_utils import preprocess_mask
...
class Tracker:
    ...
    def run(self, stream:BaseDataset):
        ...
        for i in range(len(stream)):
            timestamp, image, _, _, mask = stream[i]
            with torch.no_grad():
                if mask is not None:
                    conf_msk, conf_msk_small = preprocess_mask(mask[0], 
                                                               H=self.cfg["cam"]["H_out"], 
                                                               W=self.cfg["cam"]["W_out"]) ##
                else:
                    conf_msk_small = None
                    conf_msk = None
                
                ### check there is enough motion
                self.motion_filter.track(timestamp, image, intrinsic, conf_msk_small, conf_msk)
